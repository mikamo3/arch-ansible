...
show_help() {
    ...
    echo "  shell        - Shell environment and CLI utilities"
    echo "  devices      - Hardware-specific drivers and device support"
    echo "  desktop      - Desktop environment foundation"
    echo "  storage      - Storage, backup, and encryption tools"
    ...
    echo "  $0 --role shell         # shellロールのみ実行"
    echo "  $0 --role base --check  # baseロールのドライラン"
    echo "  $0 --tags base,shell    # base と shell ロールを実行"
    echo "  $0 --tags init          # initロール実行（システムインストール）"
    echo "  $0 --skip-tags desktop  # desktop以外のロールを実行"
    echo ""
}
...
ansible-playbook -i inventories/sandbox.yml playbook/main.yml "${OPTS[@]}" --vault-password-file .vault_pass --extra-vars "@password.yml"
